...
add_node_config() {
    echo -e "${green} Chọn core muốn dùng：${plain}"
    ...
    if [ "$isreality" != "y" ] && [ "$isreality" != "Y" ]; then
        read -rp "Có cấu hình TLS không? (y/n)" istls
...
EOF
    ...
    echo -e "${green}Tệp cấu hình ZicBoard được tạo và dịch vụ đang được khởi động lại.${plain}"
    v2bx restart
}
